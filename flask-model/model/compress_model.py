...
def compress_one_image(
    ...
    metrics = {
        ...
        "original_size": f"{original_img_size / 1024:.2f}",
        "compressed_size": f"{compressed_img_size / 1024:.2f}",
    }
    metrics.update(compression_metrics)

    # return metrics as well as the compressed image
    return metrics, img_bytes
...
class ImageCompressor:
    ...
    def calculate_metrics(self):
        metrics = {
            "PSNR": "{:.4f}".format(compute_psnr(self.image_tensor, self.out_net['x_hat'])),
            "MS-SSIM": "{:.4f}".format(compute_msssim(self.image_tensor, self.out_net['x_hat'])),
            "Bit-rate": "{:.4f}".format(compute_bpp(self.out_net)),
        }
        return metrics
